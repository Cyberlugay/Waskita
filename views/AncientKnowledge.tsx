    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-20 px-0 md:px-10 pt-8 bg-[var(--bg-primary)] min-h-screen text-[var(--text-primary)]">
      <header className="space-y-4 px-4">
        ...
        <div className="flex items-center gap-3">
          <div className="p-3 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl text-[var(--pasundan-green-dark)] shadow-sm">
            <Skull size={32} />
          </div>
          <div>
            <h2 className="text-3xl md:text-5xl font-heritage font-bold text-[var(--text-primary)] uppercase tracking-tight leading-none">Ilmu Leluhur</h2>
            <p className="text-[var(--pasundan-green-dark)] uppercase text-[10px] tracking-[0.4em] font-black mt-2">Simulasi Kebatinan & Mantra Kuno Nusantara</p>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 px-0 md:px-0">
        ...
        <div className="lg:col-span-5 space-y-8 px-4 md:px-0 flex flex-col h-full">
          <div className="p-8 bg-[var(--bg-card)] rounded-[40px] border border-[var(--border-color)] space-y-8 shadow-sm flex-1">
            <div className="flex gap-2 p-1 bg-[var(--bg-secondary)] rounded-2xl border border-[var(--border-color)]">
              {(['Pelet', 'Puter Giling', 'Santet'] as const).map(cat => (
                <button 
                  ...
                  className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${category === cat ? 'bg-[var(--pasundan-green)] text-white shadow-md' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
                >
                  {cat}
                </button>
              ))}
            ...
            {/* Category Context Info */}
              <div className={`p-6 ${categoryDetails.bg} rounded-3xl border border-[var(--border-color)] space-y-4 animate-in fade-in duration-700`}>
              ...
              <p className="text-xs text-[var(--text-secondary)] italic leading-relaxed">"{categoryDetails.desc}"</p>
              <div className="space-y-3 pt-2">
                <div className="flex gap-3">
                   <Zap size={14} className={`${categoryDetails.color} shrink-0 mt-0.5`} />
                   <div className="space-y-1">
                     <p className="text-[9px] font-black text-[var(--text-primary)] uppercase tracking-wider">Filosofi Batin</p>
                     <p className="text-[10px] text-[var(--text-secondary)] leading-tight">{categoryDetails.philosophy}</p>
                   </div>
                </div>
                <div className="flex gap-3">
                   <Layers size={14} className={`${categoryDetails.color} shrink-0 mt-0.5`} />
                   <div className="space-y-1">
                     <p className="text-[9px] font-black text-[var(--text-primary)] uppercase tracking-wider">Tata Cara Tradisi</p>
                     <p className="text-[10px] text-[var(--text-secondary)] leading-tight">{categoryDetails.ritual}</p>
                   </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full bg-[var(--bg-surface-soft)] border border-[var(--border-color)] rounded-xl p-4 text-[var(--text-primary)] focus:border-[var(--pasundan-green)] outline-none text-sm font-bold shadow-inner placeholder:text-[var(--text-muted)]" 
                />
              </div>
        ...
        <div className="lg:col-span-7 space-y-8 px-0 md:px-0 pb-10">
          <div className="p-0 md:p-12 bg-[var(--bg-card)] md:rounded-[40px] border-y md:border border-[var(--border-color)] min-h-[500px] flex flex-col shadow-sm relative overflow-hidden">
            <div className="flex items-center gap-3 text-[var(--pasundan-green)] mb-8 border-b border-[var(--border-color)] pb-6 pt-8 px-6 md:pt-0 md:px-0 relative z-10">
            ...
            <div className="flex-1 overflow-y-auto px-0 md:px-0 space-y-10 relative z-10">
                  ...
                  >
                    {!image && !isCameraActive && (
                      <div className="text-center p-8 space-y-6">
                        <div className="w-20 h-20 bg-[var(--bg-card)] rounded-3xl flex items-center justify-center text-[var(--text-muted)] mx-auto shadow-inner border border-[var(--border-color)]">
                          <Camera size={40} />
                        </div>
                        <div className="space-y-2">
                        ...
                        <div className="flex gap-4 w-full max-w-xs mx-auto">
                          <button 
                            onClick={() => startCamera()}
                            className="flex-1 py-4 bg-[var(--pasundan-green)] text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-sm active:scale-95 transition-transform"
                          >
                            KAMERA
                          </button>
                          <button 
                            onClick={() => fileInputRef.current?.click()}
                            className="flex-1 py-4 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl text-[var(--text-secondary)] text-[10px] font-black shadow-sm uppercase tracking-widest hover:bg-[var(--bg-soft)] transition-colors"
                          >
                            UNGGAH
                          </button>
                        </div>
              ...
              {analysis && (
                <div className="space-y-12 animate-in fade-in duration-1000 w-full px-0 pb-10">
                      ...
                      <div className="relative p-1 bg-stone-900 border border-stone-800 rounded-[40px] overflow-hidden shadow-[0_0_50px_-10px_rgba(220,38,38,0.3)] mx-auto flex flex-col group transition-all duration-1000">
                         ...
                         <div className="p-8 text-center border-t border-stone-800 bg-stone-950 relative z-20 flex justify-between items-center">
                          ...
                          <button 
                              ...
                              className="flex items-center gap-2 px-6 py-2.5 bg-[var(--pasundan-green)] hover:bg-[var(--pasundan-green-dark)] text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-sm transition-all active:scale-95"
                            >
                  ...
                  <div className="px-4 md:px-0">
                    <div className="text-[var(--text-primary)] text-lg md:text-3xl leading-relaxed italic text-justify whitespace-pre-wrap bg-[var(--bg-surface-soft)] p-6 md:p-20 border-y md:border md:rounded-[40px] border-[var(--border-color)] shadow-sm w-full font-medium animate-in slide-in-from-bottom-4 duration-1000">
                      {analysis}
                    </div>
                  </div>
