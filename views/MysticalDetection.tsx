    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-20 px-0 md:px-10 pt-8 bg-[var(--bg-primary)] min-h-screen text-[var(--text-primary)]">
      <header className="space-y-4 px-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-4xl md:text-5xl font-heritage font-bold text-[var(--text-primary)] tracking-tight uppercase">Deteksi Energi</h2>
            <p className="text-[var(--text-secondary)] uppercase text-[10px] tracking-[0.4em] font-black mt-1">Pemindaian Anomali & Entitas Ghaib</p>
          </div>
        </div>
      </header>
          ...
          <div className="aspect-[3/4] bg-[var(--bg-secondary)] rounded-[40px] border-2 border-dashed border-[var(--border-color)] overflow-hidden relative group shadow-inner">
            ...
            {originalImage && (
              <div className="absolute inset-0 group">
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-[var(--bg-card)]/40 backdrop-blur-[1px]">
                  <p className="text-[var(--pasundan-green)] text-[10px] font-black uppercase tracking-[0.3em] animate-pulse">Dimensi Terbuka</p>
                </div>
                <button onClick={() => { setOriginalImage(null); setGeneratedImage(null); startCamera(); }} className="absolute top-4 right-4 p-3 bg-[var(--bg-card)]/80 rounded-full text-[var(--text-primary)] shadow-sm hover:bg-[var(--pasundan-green)] transition-all opacity-0 group-hover:opacity-100"><RefreshCw size={18} /></button>
              </div>
            )}
            <canvas ref={canvasRef} className="hidden" />
            <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleImageUpload} />
          </div>
          <button onClick={() => performScan()} disabled={!originalImage || loading} className="w-full py-6 bg-[var(--bg-secondary)] hover:bg-[var(--bg-card)] text-[var(--text-primary)] font-black rounded-2xl shadow-sm transition-all uppercase tracking-widest text-[11px]">
            {loading ? <Loader2 className="animate-spin" size={16} /> : <Sparkles size={16} className="text-[var(--pasundan-green)]" />} {loading ? 'MENEMBUS DIMENSI...' : 'INISIASI TERAWANGAN'}
          </button>
        </div>

        <div className="space-y-6 pb-10">
          <div className="p-0 md:p-16 glass-panel md:rounded-[60px] border border-[var(--border-color)] bg-[var(--bg-card)] h-full min-h-[500px] flex flex-col shadow-sm relative overflow-hidden">
            ...
            <div className="flex-1 w-full px-0">
              {!analysis && !loading && (
                <div className="h-full flex flex-col items-center justify-center text-[var(--text-muted)] space-y-6 opacity-30 py-20">
                  <Ghost size={80} />
              ...
              {loading && (
                <div className="h-full flex flex-col items-center justify-center space-y-8 py-20 px-6 text-center">
                  <div className="relative">
                    <div className="w-24 h-24 border-2 border-[var(--pasundan-green-light)] rounded-full animate-ping"></div>
                    <Radio className="absolute inset-0 m-auto text-[var(--pasundan-green)] animate-pulse" size={40} />
                  </div>
                  <p className="text-[var(--pasundan-green)] font-heritage italic text-2xl animate-pulse">Menghimpun resonansi batin...</p>
                </div>
              )}
              {analysis && (
                <div className="space-y-12 animate-in fade-in duration-1000 px-0">
                  <div className="text-[var(--text-primary)] leading-relaxed italic text-justify text-lg md:text-4xl whitespace-pre-wrap font-medium p-6 px-[1px] md:p-20 bg-[var(--bg-secondary)] md:rounded-[40px] border-y md:border border-[var(--border-color)] shadow-sm w-full">
                    {analysis}
                  </div>
          ...
          <div className="p-6 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-3xl flex gap-4 mx-4 md:mx-0">
            <AlertCircle className="text-[var(--text-muted)] shrink-0" size={20} />
            <div className="text-[10px] text-[var(--text-secondary)] leading-relaxed italic font-medium">
              <span className="font-bold text-[var(--text-primary)] block mb-1 uppercase tracking-widest">Peringatan Etika</span>
