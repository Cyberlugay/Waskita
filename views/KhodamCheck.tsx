...
import { Camera, Upload, Loader2, Sparkles, RefreshCw, Eye, Ghost, SwitchCamera, Home, User, Flame, Zap, Calendar, Heart, Download, ScrollText, Layers } from 'lucide-react';
...
const KhodamCheckView: React.FC<{ onNavigate: (view: AppView) => void }> = ({ onNavigate }) => {
  ...
  const handleAnalyze = async () => {
    if (!image || !name.trim() || !birthDate.trim() || !motherName.trim()) {
      alert("Mohon lengkapi seluruh data batin.");
      return;
    }
    setLoading(true);
    setAnalysis('');
    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-20 px-0 md:px-10 pt-8 bg-[var(--bg-primary)] min-h-screen text-[var(--text-primary)]">
      <header className="space-y-4 px-4">
        <button 
          ...
          className="flex items-center gap-2 text-[var(--text-secondary)] hover:text-[var(--title-primary)] transition-colors mb-6 group"
        >
        ...
        <div className="flex items-center gap-4">
          <div className="p-4 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl text-[var(--title-primary)] shadow-sm">
            <Layers size={32} />
          </div>
          <div>
            <h2 className="text-4xl md:text-5xl font-heritage font-bold text-[var(--title-primary)] uppercase tracking-tight">Cek Khodam</h2>
            <p className="text-[var(--title-primary)] uppercase text-[10px] tracking-[0.4em] font-black mt-1">Kartu Lukisan Batin Nusantara</p>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="space-y-8 px-4">
          <div className="p-8 bg-[var(--bg-card)] rounded-[40px] border border-[var(--border-color)] space-y-8 shadow-sm">
            <div className="grid grid-cols-1 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest flex items-center gap-2">
                  <User size={12} className="text-[var(--title-primary)]" /> Nama Lengkap Sesuai Sanad
                </label>
                <input 
                  type="text" 
                  value={name} 
                  ...
                  className="w-full bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl p-4 text-[var(--text-primary)] focus:border-[var(--title-primary)] outline-none text-sm font-bold shadow-inner placeholder:text-[var(--text-muted)]" 
                />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest flex items-center gap-2">
                    <Calendar size={12} className="text-[var(--title-primary)]" /> Tgl Lahir (HH-BB-TTTT)
                  </label>
                  <input 
                    type="text" 
                    value={birthDate} 
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest flex items-center gap-2">
                    <Heart size={12} className="text-[var(--title-primary)]" /> Nama Ibu Kandung
                  </label>
                  <input 
                    type="text" 
                    value={motherName} 
            ...
            <div 
              className={`
                aspect-[3/4] rounded-[40px] border-2 border-dashed flex flex-col items-center justify-center transition-all overflow-hidden relative bg-[var(--bg-secondary)]
                ${image || isCameraActive ? 'border-[var(--pasundan-green)]' : 'border-[var(--border-color)]'}
              `}
            >
              {!image && !isCameraActive && (
                <div className="text-center p-8 space-y-6">
                  ...
                  <div className="space-y-2">
                    <p className="text-stone-300 font-bold uppercase tracking-widest text-[10px]">Pindai Citra Batin</p>
                    <p className="text-stone-600 text-[10px] italic">Tangkap aura wajah Anda.</p>
                  </div>
                  <div className="flex gap-4 w-full max-w-xs mx-auto">
                    <button 
                      onClick={() => startCamera()}
                      className="flex-1 py-4 bg-amber-600 text-stone-950 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-2xl active:scale-95 transition-transform"
                    >
                      KAMERA
                    </button>
                    <button 
                      onClick={() => fileInputRef.current?.click()}
                      className="flex-1 py-4 bg-stone-800 border border-stone-700 rounded-xl text-stone-300 text-[10px] font-black shadow-sm uppercase tracking-widest hover:bg-stone-700 transition-colors"
                    >
                      UNGGAH
                    </button>
                  </div>
              ...
              {isCameraActive && (
                <div className="absolute inset-0">
                  <video 
                    ...
                    autoPlay 
                    playsInline 
                    className={`w-full h-full object-cover grayscale opacity-60 ${facingMode === 'user' ? 'scale-x-[-1]' : ''}`} 
                  />
                  <div className="absolute bottom-8 left-0 right-0 flex justify-center items-center gap-6 z-20">
                    <button onClick={toggleCamera} className="p-4 bg-black/40 backdrop-blur-md rounded-full text-stone-300 border border-stone-700 transition-all hover:bg-stone-800">
                      <SwitchCamera size={24} />
                    </button>
                    <button onClick={capturePhoto} className="w-16 h-16 bg-amber-600 rounded-full flex items-center justify-center text-stone-950 shadow-lg active:scale-90 transition-transform">
                      <Flame size={28} />
                    </button>
                    <button onClick={stopCamera} className="p-4 bg-black/40 backdrop-blur-md rounded-full text-stone-300 border border-stone-700 transition-all hover:bg-rose-900">
                      <RefreshCw size={24} />
                    </button>
                  </div>
                </div>
              ...
              {image && (
                <div className="absolute inset-0 group">
                  ...
                  <button 
                    ...
                    className="absolute top-6 right-6 p-4 bg-stone-900/80 backdrop-blur rounded-full text-stone-100 shadow-xl hover:bg-amber-600 transition-all opacity-0 group-hover:opacity-100"
                  >
                    <RefreshCw size={20} />
                  </button>
                </div>
            ...
            <button 
              ...
              disabled={!image || !name.trim() || !birthDate.trim() || !motherName.trim() || loading}
              className="w-full py-6 bg-stone-100 hover:bg-white text-stone-950 font-black rounded-2xl transition-all flex items-center justify-center gap-3 disabled:opacity-50 shadow-2xl uppercase tracking-widest text-[11px]"
            >
        ...
        <div className="space-y-6 px-0 md:px-0">
          <div className="p-0 md:p-12 glass-panel md:rounded-[60px] border-y md:border border-stone-800 bg-stone-900/30 min-h-[500px] flex flex-col shadow-2xl">
            <div className="flex items-center gap-3 text-amber-500 mb-8 border-b border-stone-800 pb-6 pt-8 px-6 md:pt-0 md:px-0">
              ...
              {loading && (
                <div className="h-full flex flex-col items-center justify-center space-y-8 py-20 px-6">
                  <div className="relative">
                    ...
                    <Flame className="absolute inset-0 m-auto text-amber-600 animate-pulse" size={40} />
                  </div>
                  <p className="text-amber-600 font-heritage italic text-xl animate-pulse text-center">Melukis sanad energi...</p>
                </div>
              )}
