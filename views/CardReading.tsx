    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-20 px-0 md:px-10 pt-8 bg-[var(--bg-primary)] min-h-screen text-[var(--text-primary)]">
      <header className="space-y-4 px-4">
        ...
        <div className="flex items-center gap-4">
          <div className="p-4 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl text-[var(--pasundan-green)] shadow-sm"><Layers size={32} /></div>
          <div>
            <h2 className="text-4xl md:text-5xl font-heritage font-bold text-[var(--text-primary)] tracking-tight uppercase">Kartu Waskita</h2>
            <p className="text-[var(--text-secondary)] uppercase text-[10px] tracking-[0.4em] font-black mt-1">Refleksi Simbolik Perjalanan Hidup</p>
          </div>
        </div>
      </header>
      ...
      {!readingStarted ? (
        <div className="max-w-4xl mx-auto space-y-10 px-4">
          <div className="p-8 md:p-12 bg-[var(--bg-card)] rounded-[40px] border border-[var(--border-color)] space-y-8 shadow-sm">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 border-b border-[var(--border-color)] pb-8">
        ...
        <div className="space-y-16 pb-20">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 px-4">
            {selectedCards.map((card, idx) => (
              <div key={idx} className={`flex flex-col items-center gap-6 transition-all duration-1000 ${activeCardIndex === idx ? 'scale-105 z-30' : 'opacity-80 scale-95'}`} onClick={() => setActiveCardIndex(idx)}>
                <div className={`px-6 py-2 border rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] ${activeCardIndex === idx ? 'bg-amber-600 border-amber-400 text-stone-950 shadow-xl' : 'bg-stone-900 border-stone-800 text-amber-400'}`}>{selectedSpread.positions[idx]}</div>
                <div className="w-full aspect-[3/4] md:aspect-[4/5] rounded-[40px] overflow-hidden border border-stone-800 bg-stone-900 shadow-2xl flex flex-col items-center justify-center relative">
                  {card.imageLoading ? (
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <span className="text-[10px] text-amber-300 font-bold uppercase tracking-widest animate-pulse">Menenun Kartu...</span>
                    </div>
                  ) : (
                    <img src={card.image!} alt={card.name} className="w-full h-full object-cover brightness-90 contrast-110" />
                  )}
                  <div className="absolute bottom-6 left-0 right-0 text-center z-20 px-4">
                    <h4 className="text-2xl font-heritage font-bold text-white text-glow-amber">{card.name}</h4>
                    <p className="text-xs text-amber-200 mt-1 italic font-medium">{card.meaning}</p>
                  </div>
          ...
          <div className="p-0 md:p-16 glass-panel md:rounded-[60px] border-y md:border border-stone-800 bg-stone-900/30 shadow-2xl">
            <div className="flex items-center gap-4 text-amber-500 mb-10 border-b border-stone-800 pb-8 px-6 pt-8 md:pt-0 md:px-0">
               <Sparkles size={28} className="animate-pulse" /><h3 className="font-heritage text-3xl md:text-5xl font-bold uppercase tracking-wider">Risalah Waskita Nusantara</h3>
            </div>
            <div className="bg-stone-950/50 p-6 md:p-20 md:rounded-[40px] border-y md:border border-stone-800 shadow-inner space-y-8">
               <div className="text-stone-100 leading-relaxed italic text-justify text-lg md:text-3xl whitespace-pre-wrap font-medium">{interpretation || (loading && "Menenun tabir rahasia kartu...")}</div>
               
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-8 border-t border-stone-800">
                 {selectedCards.map((c, i) => (
                   <div key={i} className="p-6 bg-stone-900/80 rounded-2xl border border-stone-800 space-y-2">
                     <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest">{selectedSpread.positions[i]}</span>
                     <h5 className="text-xl font-heritage font-bold text-white">{c.name}</h5>
                     <p className="text-sm text-stone-300 italic">{c.meaning}</p>
                   </div>
                 ))}
               </div>
