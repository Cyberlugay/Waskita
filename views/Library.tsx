...
const LibraryView: React.FC<{ onNavigate: (view: AppView) => void }> = ({ onNavigate }) => {
  ...
  if (readingManuscript) {
    return (
      <div className="space-y-6 bg-stone-50 min-h-screen animate-in fade-in duration-500 pb-20 px-4 pt-8 text-stone-900">
        <div className="flex items-center justify-between py-6">
          <button 
            onClick={() => setReadingManuscript(null)}
            className="flex items-center gap-2 text-stone-600 hover:text-emerald-800 transition-colors"
          >
            <ArrowLeft size={20} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-1">
            <div className="aspect-[3/4] rounded-[40px] overflow-hidden border border-stone-200 shadow-sm">
              <img src={readingManuscript.image} className="w-full h-full object-cover" alt={readingManuscript.title} />
            </div>
            <div className="mt-6 p-6 bg-white rounded-3xl border border-stone-100">
              <h5 className="text-[10px] font-black text-emerald-800 uppercase tracking-widest mb-2">Asal Sanad</h5>
              <p className="text-stone-900 font-bold text-sm">{readingManuscript.origin}</p>
            </div>
          </div>

          <div className="lg:col-span-3 space-y-10">
            <section className="p-6 md:p-16 bg-white border border-stone-100 rounded-[40px] shadow-sm">
              <h4 className="text-3xl font-heritage font-bold text-stone-900 border-b border-stone-100 pb-6 mb-8">Sinopsis Tekstual</h4>
              <p className="text-stone-700 text-lg md:text-2xl leading-relaxed italic text-justify font-medium">
                {readingManuscript.fullContent}
              </p>
            </section>

            <section className="p-6 md:p-16 bg-white border border-stone-100 rounded-[40px] shadow-sm">
              <div className="flex items-center gap-3 text-emerald-800 mb-8 border-b border-stone-100 pb-6">
              ...
              {loading ? (
                <div className="py-20 flex flex-col items-center justify-center space-y-6">
                  ...
                  <p className="text-emerald-800 font-heritage italic text-lg animate-pulse">Menerjemahkan sanad batin...</p>
                </div>
              ) : (
                <div className="space-y-8 animate-in fade-in duration-1000">
                  <div className="text-stone-900 text-lg md:text-3xl leading-relaxed italic text-justify whitespace-pre-wrap bg-stone-50 p-8 md:p-16 rounded-[40px] border border-stone-100 shadow-inner font-medium">
                    {aiAnalysis}
                  </div>
  ...
  return (
    <div className="space-y-12 bg-stone-50 pt-8 pb-20 px-4 md:px-10 text-stone-900 min-h-screen">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-6">
          <button 
            onClick={() => onNavigate(AppView.HOME)}
            className="flex items-center gap-2 text-stone-600 hover:text-emerald-800 transition-colors group"
          >
          ...
          <div className="flex items-center gap-4">
             <div className="p-4 bg-white border border-stone-200 rounded-2xl text-emerald-800 shadow-sm"><BookOpen size={28} /></div>
             <div>
               <h2 className="text-4xl md:text-5xl font-heritage font-bold text-stone-900 tracking-tight">Pustaka Naskah</h2>
        ...
        <div className="relative w-full md:w-[450px]">
          <input 
            ...
            className="w-full bg-white border border-stone-200 rounded-2xl py-4 px-6 pl-12 text-stone-900 focus:border-emerald-600 text-sm outline-none shadow-sm placeholder:text-stone-400"
          />
