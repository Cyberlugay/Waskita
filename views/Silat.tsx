...
const SilatView: React.FC<{ onNavigate: (view: AppView) => void }> = ({ onNavigate }) => {
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-500 pb-20 px-0 pt-8 bg-[var(--bg-primary)] min-h-screen text-[var(--text-primary)]">
      <header className="space-y-8 px-4 md:px-10">
        ...
        <div className="flex items-center gap-4">
          <div className="p-4 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-3xl text-[var(--pasundan-green)] shadow-sm">
            <Shield size={32} />
          </div>
          <div>
            <h2 className="text-4xl md:text-6xl font-heritage font-bold text-[var(--text-primary)] tracking-tight uppercase">Pencak Silat</h2>
            <p className="text-[var(--text-secondary)] uppercase text-[10px] md:text-xs tracking-[0.4em] font-black mt-2">Penjaga Marwah & Warisan Agung Nusantara</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 px-0 md:px-10">
        <div className="lg:col-span-4 space-y-3 px-4 md:px-0">
          <div className="px-2 pb-2">
            <h3 className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em]">Khasanah {sections.find(s => s.id === activeSection)?.label}</h3>
          </div>
          <div className="space-y-3">
            {currentData.map((item) => (
              <button
                ...
                className={`w-full text-left p-6 bg-[var(--bg-card)] rounded-3xl border transition-all flex items-center gap-5 group shadow-sm ${selectedId === item.id ? 'border-[var(--pasundan-green)] bg-[var(--bg-secondary)] ring-1 ring-[var(--pasundan-green)]' : 'border-[var(--border-color)] hover:border-[var(--border-color)]'}`}
              >
                <div className={`p-4 rounded-2xl transition-all ${selectedId === item.id ? 'bg-[var(--pasundan-green)] text-white shadow-sm' : 'bg-[var(--bg-secondary)] text-[var(--text-secondary)]'}`}>
                  {item.icon}
                </div>
                <div className="flex-1">
                  <h4 className="font-heritage text-xl font-bold text-[var(--title-primary)] leading-tight group-hover:text-[var(--title-dark)] transition-colors">{item.name}</h4>
                  <p className="text-[10px] text-stone-600 uppercase font-black tracking-widest mt-1 italic">{item.context}</p>
                </div>
                <ChevronRight size={20} className={`transition-all ${selectedId === item.id ? 'translate-x-1 text-[var(--title-primary)]' : 'opacity-0 group-hover:opacity-100 text-stone-700'}`} />
              </button>
            ))}
          </div>
        </div>
          ...
          {selectedItem ? (
            <div className="space-y-0 animate-in slide-in-from-right duration-700 px-0 pb-10">
               <div className="mb-10 px-4 md:px-0">
                  <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-[var(--pasundan-green-light)] text-[var(--pasundan-green)] border border-[var(--pasundan-green)]/30 rounded-full text-[9px] font-black uppercase tracking-widest mb-4">
                    Kategori: {selectedItem.category}
                  </div>
                  <h3 className="text-4xl md:text-7xl font-heritage font-bold text-[var(--title-primary)] tracking-tight leading-none mb-6">{selectedItem.name}</h3>
                  <p className="text-[var(--text-secondary)] text-lg md:text-xl italic font-medium max-w-3xl leading-relaxed">"{selectedItem.desc}"</p>
               </div>

               <div className="w-full px-0">
                  <div className="flex items-center gap-3 text-[var(--title-primary)] mb-8 border-b border-[var(--border-color)] pb-6 px-4 md:px-0">
                  ...
                  {loading ? (
                    <div className="py-24 flex flex-col items-center justify-center space-y-6 px-4 md:px-0">
                      <div className="relative">
                        <div className="w-20 h-20 border-4 border-[var(--border-color)] border-t-[var(--pasundan-green)] rounded-full animate-spin"></div>
                        <Shield className="absolute inset-0 m-auto text-[var(--pasundan-green)]" size={24} />
                      </div>
                      <p className="text-[var(--pasundan-green)] font-heritage italic text-xl animate-pulse">Meresonansi sanad batin...</p>
                    </div>
                  ) : (
                    <div className="space-y-12 px-0">
                      <div className="text-[var(--text-primary)] text-lg md:text-3xl leading-relaxed italic text-justify whitespace-pre-wrap w-full bg-[var(--bg-secondary)] md:p-16 p-6 px-[1px] md:rounded-[40px] border-y md:border border-[var(--border-color)] shadow-sm font-medium">
