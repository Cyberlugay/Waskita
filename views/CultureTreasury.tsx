...
const CultureTreasuryView: React.FC<{ onNavigate: (view: AppView) => void }> = ({ onNavigate }) => {
  ...
  const handleOpenItem = async (item: CulturalItem) => {
    ...
    getCulturalSynthesis(`Risalah filosofis mendalam tentang "${item.title}". Gunakan gaya puitis waskita polos.`).then(setAiInsight);
  ...
  return (
    <div className="space-y-12 bg-[var(--bg-primary)] pt-8 pb-20 px-4 md:px-10 min-h-screen text-[var(--text-primary)]">
      {!selectedItem ? (
        <>
          <header className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
              <div className="flex items-center gap-4"><div className="p-4 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl text-[var(--pasundan-green)] shadow-sm"><Palette size={28} /></div><div><h2 className="text-4xl md:text-5xl font-heritage font-bold text-[var(--text-primary)] tracking-tight uppercase">Seni & Budaya</h2><p className="text-[var(--text-secondary)] uppercase text-[10px] tracking-[0.4em] font-black mt-1">Situs & Kemaharajaan Nusantara</p></div></div>
              <div className="relative w-full md:w-[450px]"><input type="text" placeholder="Cari..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-[var(--bg-surface-soft)] border border-[var(--border-color)] rounded-2xl py-4 px-6 pl-12 text-[var(--text-primary)] focus:border-[var(--pasundan-green)] outline-none shadow-inner placeholder:text-[var(--text-muted)]" /><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--text-secondary)]" size={20} /></div>
            </div>
            <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hide">{categories.map(cat => (<button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className={`flex shrink-0 items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${selectedCategory === cat.id ? 'bg-[var(--pasundan-green)] text-white border-[var(--pasundan-green)] shadow-md' : 'bg-[var(--bg-surface-soft)] border-[var(--border-color)] text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}>{cat.icon} {cat.label}</button>))}</div>
          </header>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">{filteredItems.map((item) => (
            <div key={item.id} onClick={() => handleOpenItem(item)} className="group bg-[var(--bg-card)] rounded-[40px] border border-[var(--border-color)] overflow-hidden hover:border-[var(--pasundan-green)] transition-all cursor-pointer shadow-sm flex flex-col h-full">
              <div className="h-64 relative overflow-hidden"><img src={item.image} alt={item.title} className="w-full h-full object-cover group-hover:scale-110 transition-all duration-1000" /><div className="absolute top-4 left-4 flex gap-2"><div className="px-3 py-1 bg-[var(--pasundan-green)] text-white rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm">{item.category}</div></div></div>
              <div className="p-8 flex-1 flex flex-col space-y-4"><h3 className="text-2xl font-heritage font-bold text-[var(--text-primary)] group-hover:text-[var(--pasundan-green)] transition-colors leading-tight">{item.title}</h3><p className="text-[var(--text-secondary)] text-xs italic line-clamp-2">"{item.description}"</p><div className="mt-auto pt-4 border-t border-[var(--border-color)] flex items-center justify-between"><span className="text-[9px] font-black text-[var(--pasundan-green-dark)] uppercase tracking-widest italic">Singkap Waskita</span><ChevronRight size={18} className="text-[var(--text-muted)] group-hover:text-[var(--pasundan-green)] transition-all transform group-hover:translate-x-1" /></div></div>
            </div>
          ))}</div>
        </>
      ) : (
