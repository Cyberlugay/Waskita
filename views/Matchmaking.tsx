    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-20 bg-[var(--bg-primary)] min-h-screen pt-8 text-[var(--text-primary)]">
      <header className="px-4 md:px-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-4xl md:text-5xl font-heritage font-bold text-[var(--text-primary)] tracking-tight uppercase">Petung Jodoh</h2>
            <p className="text-[var(--text-secondary)] uppercase text-[10px] tracking-[0.4em] font-black mt-1">Sinkronisasi Kosmologis Dua Jiwa</p>
          </div>
        </div>
      </header>
        ...
        <div className="p-8 md:p-12 bg-[var(--bg-card)] rounded-[40px] border border-[var(--border-color)] space-y-8 shadow-sm">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             <div className="space-y-2">
               <label className="text-[9px] font-black uppercase tracking-widest text-[var(--text-secondary)]">Pihak I (HH-BB-TTTT)</label>
               <input type="text" value={date1} onChange={(e) => setDate1(e.target.value)} placeholder="Contoh: 17-08-1945" className="w-full bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl p-4 text-[var(--text-primary)] focus:border-[var(--pasundan-green)] outline-none shadow-inner placeholder:text-[var(--text-muted)]" />
             </div>
             <div className="space-y-2">
               <label className="text-[9px] font-black uppercase tracking-widest text-[var(--text-secondary)]">Pihak II (HH-BB-TTTT)</label>
               <input type="text" value={date2} onChange={(e) => setDate2(e.target.value)} placeholder="Contoh: 20-05-1948" className="w-full bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl p-4 text-[var(--text-primary)] focus:border-[var(--pasundan-green)] outline-none shadow-inner placeholder:text-[var(--text-muted)]" />
             </div>
           </div>
           <button onClick={handleAnalyze} disabled={loading || !date1 || !date2} className="w-full py-6 bg-[var(--pasundan-green)] hover:bg-[var(--pasundan-green-dark)] text-white font-black rounded-2xl transition-all flex items-center justify-center gap-3 shadow-sm uppercase tracking-widest text-[11px]">
             {loading ? <Loader2 className="animate-spin" size={18} /> : <Sparkles size={18} />} SINGKAP TABIR JODOH
           </button>
        </div>
